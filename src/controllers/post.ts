...
const likeComment = async(req: IGetUserAuthInfoRequest, res: Response) => {
  const post = await postlib.likecomment(req.user, req.params.id)
  res.status(200).send(post)
}
...
const replyToComment = async(req: IGetUserAuthInfoRequest, res: Response) => {
  const post = await postlib.replyToComments(req.user, req.params.id, req.body)
  res.status(200).send(post)
}

export {
  createPost, sharePost, getUserFeed, deletePost, editPost, likePost,
  unlikePost, commentOnPost, replyToComment, getPosts, likeComment
}
